...
(function (global) {
  ...
  var CONFIG_PATH = 'config/mock-config.json';
  ...
  function onEndpointChange() {
    updateBanner();
    console.info('[depth-gather] 切换数据源 →', isMockSelected() ? 'Mock 本地' : '测试环境',
  ...
  function patchAjax() {
    ...
    global.jQuery.ajax = function (opts) {
      ...
      if (isDepthGatherRequest(url) && shouldMockRequest(url)) {
        ...
        console.info('[depth-gather] mock →', paths.primary);
        ...
        fetchMockJson(paths).then(function (json) {
          ...
          console.error('[depth-gather] mock failed:', err.message);
  ...
  function boot() {
    loadConfig().then(function (c) {
      ...
      tryPatch();
      global.dispatchEvent(new CustomEvent('depth-gather-config-ready', { detail: cfg }));
      var sel = getSelect();
      console.info('[depth-gather] ready mode=' + (isMockSelected() ? 'mock' : 'live') +
        ' url=' + (sel ? sel.value : DEFAULT_LIVE));
    });
  }

  global.DepthGatherMock = {
    reload: boot,
