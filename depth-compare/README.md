...
| 图表缩放 | 价格预设范围（默认 1%，绕 Binance mid 截取）/ ± 按钮 / 框选 / 滚轮 / 单档量轴缩放 |
| 买卖分界 | 各所 mid 横虚线 + bid/ask 价差带；左侧表展示 Mid / 价差 bp |
...
3. 勾选要显示的交易所
4. 点击 **定时刷新** 或 **立即刷新**
5. 价格范围默认 **1%**（绕 Binance mid）。点 0.25% / 0.5% / 2%… 会按同一锚点截取，不会因为某所盘口更宽就退回「全部」

## 怎么看买卖分界

图是纵向深度（上卖下买）。**横虚线 = 该所买卖分界（mid）**，线旁标注所名、mid 价格、价差 bp；半透明横带是 bid1～ask1 的价差区间。
...
左侧表同步给出 Mid / 价差。本所 mid 若明显偏离 Binance 横线，就是盘口锚偏了。

价格范围的锚点优先 Binance，没有 BN 再用本所。各所共用同一窗口，方便对比，不再把 5 家 mid 平均后截空。
...
|---|-------------|---------------|
| 数据源 | 仅本所 | 本所 + 4 外所 |
| 图表 | 本所买卖盘 | 多交易所叠加 + 各所买卖分界线 |
| 价格窗 | 全盘 | 默认锚点 mid ±1%，Highcharts 不按 tick 外扩 |
| 刷新 | 单接口 | 批量等待全部响应 |
